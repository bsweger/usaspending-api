"""
Creates locations from transaction_fpds and transaction_fabs

Also assigns transaction and award place of performance, and recipient location, to new Locations
Meant to fix problems in existing data from earlier data loads
Does not delete Locations that are "orphaned" (no longer being anybody's POP or location)
"""
...
SELECT
  ...
  TRIM(UPPER(REPLACE(legal_entity_state_code, '.', ''))),  -- ==> state_code
  TRIM(UPPER(legal_entity_state_name)),  -- ==> state_name
  NULL,  -- ==> state_description
  TRIM(UPPER(legal_entity_city_name)),  -- city_name
  NULL,  -- ==> city_code
  TRIM(UPPER(legal_entity_county_name)),  -- ==> county_name
  legal_entity_county_code,  -- ==> county_code
  TRIM(UPPER(legal_entity_address_line1)),  -- ==> address_line1
  TRIM(UPPER(legal_entity_address_line2)),  -- ==> address_line2
  TRIM(UPPER(legal_entity_address_line3)),  -- ==> address_line3
...
SELECT
  ...
  ref_country_code.country_name,  -- ==> country_name
  NULL,  -- ==> state_code
  TRIM(UPPER(place_of_perform_state_nam)),  -- ==> state_name
  NULL,  -- ==> state_description
  TRIM(UPPER(place_of_performance_city)),  -- ==> city_name
  NULL,  -- ==> city_code
  TRIM(UPPER(place_of_perform_county_na)),  -- ==> county_name
  NULL,  -- ==> county_code
  NULL,  -- ==> address_line1
  NULL,  -- ==> address_line2
  NULL,  -- ==> address_line3
...
SELECT
  ...
  NULL::INTEGER,  -- ==> location_id
  ref_country_code.country_name,  -- ==> country_name
  TRIM(UPPER(REPLACE(place_of_performance_state, '.', ''))),  -- ==> state_code
  NULL,  -- ==> state_name
  NULL,  -- ==> state_description
  TRIM(UPPER(place_of_perform_city_name)),  -- ==> city_name
  TRIM(UPPER(place_of_performance_locat)),  -- ==> city_code
  NULL,  -- ==> county_name
  NULL,  -- ==> county_code
  NULL,  -- ==> address_line1
  NULL,  -- ==> address_line2
...
-- fill state code from name, when appropriate
UPDATE transaction_location_data
SET    state_code = s.abbrev
FROM   state_abbrevs s
WHERE  (transaction_location_data.state_code IS NULL OR transaction_location_data.state_code = '')
AND    s.name = transaction_location_data.state_name
AND    transaction_location_data.location_country_code = 'USA'
AND    transaction_location_data.state_code IS NULL
AND    transaction_location_data.transaction_id >= ${floor}
AND    transaction_location_data.transaction_id < ${ceiling}
;


-- fill state name from code, when appropriate
UPDATE transaction_location_data
SET    state_name = s.name
FROM   state_abbrevs s
WHERE  (transaction_location_data.state_name IS NULL OR transaction_location_data.state_name = '')
AND    s.abbrev = transaction_location_data.state_code
AND    transaction_location_data.location_country_code = 'USA'
AND    transaction_location_data.state_name IS NULL
AND    transaction_location_data.transaction_id >= ${floor}
AND    transaction_location_data.transaction_id < ${ceiling}
;
...
-- assign legal entity locations
-- if a recipient has many transactions with varying locations,
-- fpds_nightly_loader assigns a location during the first load
-- and does not change it after that
-- Using MIN is meant to duplicate that
WITH txn AS (
  ...
  GROUP BY 2
  -- HAVING MIN(transaction_id) >= ${floor}
  -- AND    MIN(transaction_id) < ${ceiling}
  )
UPDATE legal_entity
SET    location_id = txn.location_id
FROM   txn
WHERE  legal_entity.legal_entity_id = txn.legal_entity_id
;
-- delete locations that no longer have any references
"""
